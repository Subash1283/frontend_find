          ...
          <div className="landing-modal-right">
            ...
            <div className={`landing-modal-form ${activeTab === 'login' ? 'active' : ''}`}>
              ...
              <button
                ...
                title={!loginEmail ? 'Enter your email address above to receive a magic link' : 'Send 1-click login link to your email'}
              >
                <i className="fas fa-paper-plane" style={{ fontSize: '0.95rem', color: loginEmail ? '#2563eb' : '#94a3b8' }}></i>
                <span>{isSendingMagicLink ? 'Sending Link to Email...' : 'Send Direct Login Link to Email'}</span>
              </button>
              {!loginEmail && (
                <small style={{ display: 'block', textAlign: 'center', fontSize: '0.72rem', color: '#94a3b8', marginTop: '5px' }}>
                   Enter your email above to enable 1-click magic link login
                </small>
              )}

              {magicLinkResult && (
                <div style={{ background: '#f0fdf4', border: '1px solid #bbf7d0', borderRadius: '12px', padding: '14px', marginTop: '12px', marginBottom: '16px', fontSize: '0.85rem', color: '#166534' }}>
